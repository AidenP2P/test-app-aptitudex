 ...
 * Supporte les streaks avec bonus progressifs et est optimisé pour les transactions gasless
 */
contract ClaimDistributor is Ownable, ReentrancyGuard {
    ...
    constructor(address _apxToken, address _owner) {
    ...
    /**
     * @dev Met à jour les multiplicateurs de streak (admin seulement)
     */
    function updateStreakMultipliers(
        bool isDaily,
        uint256[] calldata thresholds,
        uint256[] calldata multipliers
    ) external onlyOwner {
        require(thresholds.length == multipliers.length, "Arrays length mismatch");
        require(thresholds.length > 0, "Empty arrays");
        
        if (isDaily) {
            delete dailyStreakMultipliers;
            for (uint256 i = 0; i < thresholds.length; i++) {
                require(multipliers[i] >= BASIS_POINTS, "Multiplier must be >= 100%");
                dailyStreakMultipliers.push(StreakMultiplier(thresholds[i], multipliers[i]));
                emit StreakMultiplierUpdated(true, thresholds[i], multipliers[i]);
            }
        } else {
            delete weeklyStreakMultipliers;
            for (uint256 i = 0; i < thresholds.length; i++) {
                require(multipliers[i] >= BASIS_POINTS, "Multiplier must be >= 100%");
                weeklyStreakMultipliers.push(StreakMultiplier(thresholds[i], multipliers[i]));
                emit StreakMultiplierUpdated(false, thresholds[i], multipliers[i]);
            }
        }
    }
